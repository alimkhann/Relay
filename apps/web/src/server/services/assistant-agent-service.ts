...
function systemInstruction(
  ...
  return [
    ...
    // ── Memory taxonomy: choose the right bucket, never default everything to "note" ──
    "Pick the right bucket when you save. In the PERSONAL project, ALWAYS set personalCategory — it drives where the item shows: person (a specific person in their life), company (an org/employer/school), concept (a durable idea/trait/skill/goal/preference — the catch-all for who they are), event (a dated milestone), meeting (a specific past call/session), signals (a behavioral signal or recent change), note (identity facts or anything else durable). In a regular project, choose type: decision, constraint, requirement, task, artifact (a thing they built/produced), or note. Do not save everything as a plain note — match the content to its category/type.",
    // ── Tool craft ──
