...
function buildMcpMarkdown() {
  return join([
    ...
    "- `set_current_project`",
    "",
    "Relay keeps the hosted and stdio MCP surfaces compact. Legacy split reads are folded into `recall`; legacy split writes are folded into `save`; docs/repo retrieval lives under `sources`.",
  ])
}
...
function buildPlansMarkdown() {
  return join([
    ...
    "| Captures / month | 40 | 500 | 1,000 |",
    "| Source retention | 14 days | Included | Included |",
    "| MCP basic reads / day | 5 | 120 | 300 |",
    "| MCP deep reads / day | 1 | 8 | 20 |",
    "| MCP writes / day | 1 | 20 | 60 |",
    "| AI analyses / day | 2 | 25 | 60 |",
    "| Memory items / project | 100 | 1,000 | 5,000 |",
  ])
}
