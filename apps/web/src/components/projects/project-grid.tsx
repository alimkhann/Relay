        ...
        <Link key={project.id} href={`/projects/${project.id}`}>
          <div className="rounded-[var(--relay-radius)] border border-[var(--relay-line)] bg-[var(--relay-surface)] p-4 shadow-[var(--relay-shadow-sm)] transition hover:border-[var(--relay-line-strong)] hover:shadow-[var(--relay-shadow)]">
            <div className="flex items-start justify-between gap-3">
              <h3 className="text-base font-semibold tracking-tight text-[var(--relay-ink)]">{project.name}</h3>
              <span className="shrink-0 text-xs text-[var(--relay-faint)]">{project.slug}</span>
            </div>
            <p className="mt-1.5 text-sm text-[var(--relay-muted)] line-clamp-2">{project.description || "No description yet."}</p>
