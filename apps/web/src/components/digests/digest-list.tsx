  ...
  if (digests.length === 0) {
    return (
      <div className="rounded-[18px] border border-dashed border-[var(--relay-line)] bg-white/70 p-5 text-sm text-[var(--relay-muted)]">
        No session digests yet. Relay will start building durable project state after the next meaningful capture.
      </div>
    )
  }
      ...
      {digests.map((digest) => (
        <Card key={digest.id} className="p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--relay-muted)]">Digest</p>
