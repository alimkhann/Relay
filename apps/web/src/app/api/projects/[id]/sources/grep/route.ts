...
import { grepProjectSourcesSchema } from "@relay/shared"
...
import { grepProjectSources } from "@/server/services/source-service"
...
export const POST = withApiAuth(async (request: Request, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const parsed = grepProjectSourcesSchema.parse(await request.json())
  const result = await grepProjectSources(viewer.userId, id, parsed)
