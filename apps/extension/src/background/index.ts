...
const dashboardCache = new Map<string, { dashboard: ProjectDashboardPayload | null; fetchedAt: number }>()
...
async function retryRemote<T>(task: () => Promise<T>, attempts = 2): Promise<T> {
  ...
  throw lastError instanceof Error ? lastError : new Error("Relay remote request failed.")
}

function buildTrustMetadata(dashboard: ProjectDashboardPayload | null | undefined): RelayTrustMetadata {
  ...
  const updatedAt =
    timestampCandidates.sort((left, right) => new Date(right).getTime() - new Date(left).getTime())[0] ?? null
...
async function loadSessionData() {
  ...
  try {
    ...
    const nextProjectId = session.projectId && projects.some((project) => project.id === session.projectId) ? session.projectId : projects[0]?.id ?? ""
...
async function fetchProjectDashboard(projectId: string) {
  ...
  try {
    const response = await retryRemote(() => relayFetch(`/api/projects/${projectId}`))
    if (!response.ok) {
      return cached?.dashboard ?? null
    }

    const payload = (await response.json()) as { dashboard?: ProjectDashboardPayload }
...
async function resolveBoundProject(tabId: number, pageState: RelayPageState) {
  ...
  const response = await retryRemote(() => relayFetch(`/api/extension/bindings?${query.toString()}`))
  if (!response.ok) {
    return null
  }
...
async function resolveActiveProject(tabId: number, pageState: RelayPageState) {
  ...
  const fallbackProject = remote.projects.find((project) => project.id === session.projectId) ?? remote.projects[0] ?? null
...
async function rememberProjectSelection(projectId: string, tabId: number | null, pageState: RelayPageState, projectName?: string | null) {
  const updates = [
...
async function requestPageStateFromTab(tabId: number) {
  try {
    const page = ((await chrome.tabs.sendMessage(tabId, { type: "RELAY_PAGE_STATE" })) as RelayPageState | undefined) ?? { supported: false }
...
async function buildActiveProjectState(tabId: number): Promise<RelayActiveProjectState> {
  const session = await getRelaySession()
  const state = tabStates.get(tabId)
...
function scheduleRetry(tabId: number) {
  ...
  const nextDelay = state.retryDelayMs === 0 ? (REMOTE_RETRY_BACKOFF_MS[0] ?? 5_000) : (REMOTE_RETRY_BACKOFF_MS[1] ?? 15_000)
...
async function syncTabRemoteState(tabId: number, options: { force?: boolean; reason?: string } = {}) {
  const state = getOrCreateTabState(tabId)
  const session = await getRelaySession()
  ...
  try {
    const { connected, projects, activeProject, settings } = await resolveActiveProject(tabId, state.page)
    ...
    const nextStateStatus = dashboard?.stateStatus ?? state.stateStatus ?? session.stateStatus ?? null
    ...
    state.remoteStatus = state.lastSuccessfulSyncAt || state.projectId || state.stateStatus ? "stale" : "unavailable"
    scheduleRetry(tabId)
  } finally {
...
async function captureObservedChange(tabId: number, explicitProjectId?: string) {
  const state = getOrCreateTabState(tabId)
  const session = await getRelaySession()
  ...
  try {
    ...
    if (!state.projectId && !explicitProjectId) {
      await syncTabRemoteState(tabId, { force: true, reason: "capture_needs_project" })
    }
    ...
    if (result?.ok) {
      state.lastCapturedSignature = state.page.captureSignature ?? state.lastObservedSignature
      ...
      await syncTabRemoteState(tabId, { force: true, reason: "capture_complete" })
      return {
        ok: true,
...
function scheduleAutoCapture(tabId: number, options: { immediate?: boolean } = {}) {
  const state = getOrCreateTabState(tabId)
...
async function insertProjectBrief(tabId: number, explicitProjectId?: string) {
  const state = getOrCreateTabState(tabId)
  const pageState = state.page.supported ? state.page : await requestPageStateFromTab(tabId)
  ...
  if (!state.projectId && !explicitProjectId) {
    await syncTabRemoteState(tabId, { force: true, reason: "insert_needs_project" })
  }
  ...
  const kind =
    pageState.isFreshChat || (looksLikeFreshChatRoute(pageState) && (pageState.turns ?? 0) === 0)
  ...
  if (generated.status === "pending" || !generated.packet?.content) {
    return {
      ok: false,
      reason: generated.reason ?? "Relay is still preparing your project brief."
    }
  }
  ...
  await rememberProjectSelection(projectId, tabId, pageState, state.projectName)
  invalidateProjectCache(projectId)
...
chrome.tabs.onActivated.addListener((activeInfo: { tabId: number }) => {
  ...
  void syncTabRemoteState(activeInfo.tabId, { force: true, reason: "tab_focus" })

  const state = tabStates.get(activeInfo.tabId)
  if (state && state.lastObservedSignature && state.lastObservedSignature !== state.lastCapturedSignature) {
...
chrome.commands?.onCommand.addListener((command: string) => {
  ...
  void (async () => {
    ...
    try {
      const response = (await chrome.tabs.sendMessage(tab.id, { type: "RELAY_SHORTCUT_ACTION" })) as {
        ok?: boolean
        action?: "restored" | "opened" | "invoked_insert" | "already_visible" | "no_op" | "fallback"
      }

      if (response?.ok && response.action && response.action !== "fallback" && response.action !== "no_op") {
        return
      }
    } catch {
...
chrome.runtime.onMessage.addListener((message: RelayMessage, sender: { tab?: { id?: number } }, sendResponse: (response?: unknown) => void) => {
  void (async () => {
    try {
      ...
      if (message.type === "RELAY_PAGE_STATE_UPDATE" && sender.tab?.id) {
        ...
        void syncTabRemoteState(sender.tab.id, { reason: "page_state_update" })
      ...
      if (message.type === "RELAY_GET_ACTIVE_PROJECT_STATE") {
        ...
        if (state.remoteStatus !== "ready" || !state.projectOptions.length) {
          await syncTabRemoteState(tabId, { force: true, reason: "active_state_request" })
        }
      ...
      if (message.type === "RELAY_SET_ACTIVE_PROJECT") {
        ...
        const pageState = state?.page?.supported ? state.page : tabId !== null ? await requestPageStateFromTab(tabId) : ({ supported: false } satisfies RelayPageState)

        await rememberProjectSelection(message.payload.projectId, tabId, pageState, state?.projectName)
        if (state) {
        ...
        if (tabId !== null) {
          await syncTabRemoteState(tabId, { force: true, reason: "project_switch" })
          sendResponse(await buildActiveProjectState(tabId))
        } else {
          sendResponse({ ok: true })
        }
      ...
      if (message.type === "RELAY_INSERT_PROJECT_BRIEF") {
        const tabId = message.payload?.tabId ?? sender.tab?.id
        if (!tabId) {
          sendResponse({ ok: false, reason: "No supported AI tab was provided for insertion." })
          return
        }

        sendResponse(await insertProjectBrief(tabId, message.payload?.projectId))
        return
      }
      ...
      if (message.type === "RELAY_PIN_SELECTION") {
        const tabId = message.payload.tabId ?? sender.tab?.id
        if (!tabId) {
          sendResponse({ ok: false, reason: "No supported tab was provided for saving." })
          return
        }

        const state = getOrCreateTabState(tabId)
        if (!state.projectId && !message.payload.projectId) {
          await syncTabRemoteState(tabId, { force: true, reason: "pin_needs_project" })
        }

        const projectId = message.payload.projectId || getOrCreateTabState(tabId).projectId
        if (!projectId) {
        ...
        if (!response.ok) {
          sendResponse({ ok: false, reason: await readErrorResponse(response, "Save to project failed.") })
          return
        }

        await rememberProjectSelection(projectId, tabId, state.page, state.projectName)
        invalidateProjectCache(projectId)
        await syncTabRemoteState(tabId, { force: true, reason: "save_to_project" })
        sendResponse({ ok: true })
        return
      }
      ...
      if (message.type === "RELAY_CAPTURE_VISIBLE") {
        ...
        sendResponse(await captureObservedChange(tabId, message.payload.projectId))
        return
      }
      ...
      if (message.type === "RELAY_GENERATE_BOOTSTRAP") {
        ...
        if (!response.ok) {
          sendResponse({ error: await readErrorResponse(response, "Project brief generation failed.") })
          return
        }
