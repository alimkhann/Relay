...
**State:** Memory-v2 pivot (P1–P5) done, reviewed twice, smoke-tested. The full extension-UX batch (W5-ext, W2, W3, W8, W6) is now code-complete + pushed to #35. **Everything ships in #35 before cutover** (user decision). Remaining = live verification of the extension UI, then prod cutover.
...
- **W8** ✓ (`d7d9838`). Persistent shadow-DOM edge button in `apps/extension/static/relay-content.js` (`renderEdgeSaveButton`, called from the top of `renderInlineChip` + on theme change). Shows on supported chats with content (`page.turns >= 1 && !isFreshChat`), pinned to the right page edge. Unsaved → "Save & link" (`RELAY_CAPTURE_VISIBLE` {projectId}, background falls back to `sender.tab.id`); saved → "Detach" with `window.confirm` (`RELAY_SET_CHAT_ASSOCIATION_ARCHIVED`). One-time × dismisses per-chat, persisted in `chrome.storage.local` (`relay.edgeSaveDismissed`). No green border; project name escaped. Side-panel half-circle (+ its CSS) retired.
- **W6** ✓ (`d7d9838`). `.expanded .contextItemListScroll { max-height:none; overflow-y:visible }` — in the side panel the shell is the sole scroller (no nested wheel-trap); compact popup keeps the capped inner scroll.

**ALL W-tasks code-complete. Live verification still pending (do before cutover):**
- Load the unpacked build (`apps/extension/build/chrome-mv3-prod`), sign in, open a chat **with content** on each supported site → confirm the edge button appears, Save&link works, the chat flips to "Detach", detach confirms + unlinks, × hides it for that chat only. Confirm it does NOT appear on fresh/empty chats. Verify no overlap glitch with the association toast (both sit on the right/bottom-right).
...
```
PR #35 (feat/memory-v2-architecture) — ALL extension-UX W-tasks (W5-ext/W2/W3/W8/W6) are code-complete + pushed (commits 31fdb67/10b3230/dd8fa68/d7d9838); typecheck + extension build + test:stable green after each. This session = LIVE verification + cutover. Read docs/memory-v2/HANDOFF.md "NEXT-SESSION HANDOFF" first. Steps: (1) Load the unpacked build (apps/extension/build/chrome-mv3-prod), sign in, and eyes-on verify the W8 in-page edge save/detach button (shows only on chats with content; Save&link → Detach-with-confirm; × hides per-chat; no fresh-chat show; no toast overlap), the W3 tri-state auto-capture + inline-chip trees (rollup display, leaf/project/global writes, inline-chip gates showCue), W6 single-scrollbar in the expanded panel, and W5-ext (source badges/time/recency, notes CRUD, Notes tab). Use dev branch br-muddy-morning-ag55csrg with the prod RELAY_CONTENT_ENCRYPTION_KEY (vercel env pull, shred dump, throwaway apps/web/.env.local, only Alim's account + a smoke project). (2) Then prod cutover per "Production cutover": migrations 0040–0050 to prod, CRON_SECRET, embedding-backfill loop until remaining=0, RELAY_MEMORY_PIPELINE_FULL=true + RELAY_HYGIENE_DRY_RUN=true for a week; soak RELAY_PERSONAL_MEMORY_AUTOWRITE (log-only) before flipping. (3) Chrome resubmission — W8 adds a content-script DOM surface → update the listing disclosure. NOTE: Relay MCP auth was expired last session — run `npx @onrelay/wizard` if get_brief fails. Caveman mode is on.
```
